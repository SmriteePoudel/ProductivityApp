...
export const findUserByEmail = async (email) => {
  try {
    // Try MongoDB first
    if (isConnected) {
      console.log(`🔍 Searching MongoDB for user: ${email}`);
      const user = await User.findOne({ email: email.toLowerCase() });
      if (user) {
        console.log(`✅ Found user in MongoDB: ${user.name} (${user.role})`);
        return user;
      } else {
        console.log(`❌ User not found in MongoDB: ${email}`);
      }
    } else {
      console.log("⚠️ MongoDB not connected, using in-memory database");
    }
  } catch (error) {
  ...
  // Fallback to in-memory
  console.log(`🔍 Searching in-memory database for user: ${email}`);
  const inMemoryUser = users.find((user) => user.email === email);
  if (inMemoryUser) {
    console.log(
      `✅ Found user in in-memory: ${inMemoryUser.name} (${inMemoryUser.role})`
    );
  } else {
    console.log(`❌ User not found in in-memory: ${email}`);
  }
  return inMemoryUser;
};
