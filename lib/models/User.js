...
userSchema.pre("save", function (next) {
  ...
  // Update role metadata when roles change
  if (this.isModified("roles")) {
    this.roleMetadata.lastRoleChange = new Date();
    // Add to role history for each new role
    if (!this.roleHistory) {
      this.roleHistory = [];
    }
    (this.roles || []).forEach((role) => {
      if (!this.roleHistory.some((h) => h.role === role)) {
...
// Add a virtual for primaryRole (for legacy code)
